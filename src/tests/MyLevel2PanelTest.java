...
public class MyLevel2PanelTest {
	...
	public void testBothDieInGreenPool() {
		...
		fireY = 315;
		Rectangle fFeet = new Rectangle(fireX + PLAYER_W / 2 - 5, fireY + PLAYER_H - 6, 3, 6);
		if (greenPool.intersects(fFeet))
			fireAlive = false;

		waterX = 195;
		waterY = 315;
		Rectangle wFeet = new Rectangle(waterX + PLAYER_W / 2 - 5, waterY + PLAYER_H - 6, 3, 6);
		if (greenPool.intersects(wFeet))
			waterAlive = false;
	...
	public void testFireboyCannotCollectBlueDiamond() {
		...
		Rectangle gem = new Rectangle(dx, dy, dw, dh);
		if (type == 1 && gem.intersects(fb))
			collected = true; // only type 1 for fireboy
	...
	public void testWatergirlCannotCollectRedDiamond() {
		...
		Rectangle gem = new Rectangle(dx, dy, dw, dh);
		if (type == 0 && gem.intersects(wb))
			collected = true; // only type 0 for watergirl
	...
	public void testOnlyOnePlayerAtDoorDoesNotWin() {
		...
		fireX = 5;
		fireY = 50; // at fire door
		waterX = 300;
		waterY = 600; // NOT at water door
